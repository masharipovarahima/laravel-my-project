                        ...
                        @forelse($files as $file)
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('files.download', ['name' => $file->subject->name]) }}" class="btn btn-sm btn-info">Yuklab olish</a>

                                    <!-- O'chirish tugmasi -->
                                    <form action="{{ route('files.delete', ['name' => $file->subject->name]) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Rostdan ham faylni o‘chirmoqchimisiz?')">
                                            O'chirish
                                        </button>
                                    </form>
                                </td>
...
<div class="modal fade" id="uploadFileModal" tabindex="-1" aria-labelledby="uploadFileModalLabel" aria-hidden="true">
    ...
    @csrf
    <label for="subject_name">Fan tanlang:</label>
