...
@section('content')
    <div class="container mt-4">
        <h4 class="mb-4">Konferensiyani Tahrirlash</h4>
        ...
        <form action="{{ route('conferences.update', $conference) }}" method="POST">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="id" class="form-control" value="{{ $conference->id }}"
                                readonly>
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="date" name="start_date" class="form-control"
                                value="{{ old('start_date', \Carbon\Carbon::parse($conference->start_date)->format('Y-m-d')) }}"
                                required>
                            @error('start_date')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="date" name="end_date" class="form-control"
                                value="{{ old('end_date', $conference->end_date ? \Carbon\Carbon::parse($conference->end_date)->format('Y-m-d') : '') }}">
                            @error('end_date')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 mb-3">
                            <label for="created_at" class="form-label">Yaratilgan sana</label>
                            <input type="text" name="created_at" class="form-control"
                                value="{{ $conference->created_at }}" readonly>
                        </div>
                        <div class="col-md-3 mb-3">
                            <label for="updated_at" class="form-label">Yangilangan sana</label>
                            <input type="text" name="updated_at" class="form-control"
                                value="{{ $conference->updated_at }}" readonly>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card shadow-sm mb-4">
                <div class="card-body">
                    ...
                    <div id="seminars-container-edit">
                        <!-- Existing seminar fields will be populated here via JavaScript -->
                    </div>
                    <button type="button" class="btn btn-secondary mt-2" id="add-seminar-edit">Yangi Seminar
                        Qo'shish</button>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const seminarsContainer = document.getElementById('seminars-container-edit');
            let seminarCount = {{ $conference->seminars->count() ?? 0 }};
            ...
            document.getElementById('add-seminar-edit').addEventListener('click', function() {
                addSeminar();
            });
            ...
            // Populate existing seminars
            @foreach ($conference->seminars as $seminar)
                addSeminar({
                    title: "{{ $seminar->title }}",
                    start_time: "{{ \Carbon\Carbon::parse($seminar->start_time)->format('Y-m-d\TH:i') }}",
                    end_time: "{{ $seminar->end_time ? \Carbon\Carbon::parse($seminar->end_time)->format('Y-m-d\TH:i') : '' }}",
